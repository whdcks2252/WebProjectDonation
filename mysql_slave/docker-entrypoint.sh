...
master_log_file=`/engn001/mysql/bin/mysql -uroot -p'0000' -h 172.16.0.10 -S /engn001/mysql/data/mysql.sock -e"show master status\G" | grep mysql-bin`
re="[a-z]*-bin.[0-9]*"
...
master_log_pos=`/engn001/mysql/bin/mysql -uroot -p'0000' -h 172.16.0.10 -S /engn001/mysql/data/mysql.sock -e"show master  status\G" | grep Position`
...
query="change master to master_host='172.16.0.10', master_user='replUser', master_password='0000', master_log_file='${master_log_file}', master_log_pos=${master_log_pos}, master_port=3306"

mysql -uroot -p'0000' -S /engn001/mysql/data/mysql.sock -e "${query}"
mysql -uroot -p'0000' -S /engn001/mysql/data/mysql.sock -e "start slave"
/bin/bash